...
typedef struct {
  int status;           // changeable, per-device, Logan
  int port;
  struct in_addr ipaddr;
  int c;			/* current # of connections           */ // changeable, per-service, Logan
  int tc;			/* total # of connections             */ // changeable, per-service, Logan
  int maxc;			/* max # of connections, 0 = no limit */
  unsigned long long bsent;	/* bytes sent  */               // changeable, per-service, Logan
  unsigned long long breceived;	/* bytes received */        // changeable, per-service, Logan
} CHANNEL;
...
typedef struct {
  ...
  int current;			/* current channel in group            */ // changeable, per-session, Logan
  int type;			/* either GROUP_RR or GROUP_HASH       */
  CHANNEL channels[MAXCHANNELS];                                  // changeable, per-device, Logan
} GROUP;
...
typedef struct {
  ...
  int   subrelease;
  int   pid;
  GROUP groups[MAXGROUPS];          // changeable, per-service, Logan
} COMMON;
